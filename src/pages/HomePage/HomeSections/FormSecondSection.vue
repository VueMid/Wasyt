<template>
  <section id="partnership" class="form mt-[80px] px-4 hidden md:block">
    <div
      class="container form__container flex flex-col justify-center items-start rounded-[32px] py-[30px] px-[30px]"
    >
      <div class="form__top-box flex flex-col justify-center items-start">
        <h2 class="form__title text-[20px] font-bold leading-[130%] text-white">
          Форма для обратной связи
        </h2>
        <p
          class="form__text text-white/90 text-sm font-normal leading-[140%] mt-4 w-[361px]"
        >
      ...
      >
        <div class="flex flex-row justify-center items-center">
          ...
          <button
            type="submit"
            class="form__button hidden px-8 py-[14px] bg-white flex flex-row justify-center items-center gap-4 text-blue rounded-[85px] text-base font-semibold leading-[150%]"
          >
            Отправить
  ...
  methods: {
    validatedForm() {
      ...
      if (this.username.length === 0) {
        isValid = false;
        this.error.nameError = "*Enter here youname";
      } else {
        isValid = true;
